...
export async function openalex(input: Parsed, opts?: ResolveOpts): Promise<{ targets: Target[]; trace: Trace[] }> {
  ...
  try {
    ...
    loc.forEach((it: unknown) => {
      ...
      if (pdf && allowed(input, pdf)) out.push({ kind: "pdf", source: "openalex", url: pdf, confidence: 0.62 })
      if (html && allowed(input, html)) out.push({ kind: "html", source: "openalex", url: html, confidence: 0.55 })
    })
